...
# чекаємо Redis (у docker мережі хост — 'redis')
wait_for "${REDIS_HOST:-redis}" "${REDIS_PORT:-6379}"

# Alembic (за наявності DATABASE_URL)
...
# каталог для celery beat schedule, щоб уникнути Permission denied
...
case "${SERVICE:-api}" in
  api)
    ...
    exec uvicorn app:app --host "${APP_HOST:-0.0.0.0}" --port "${APP_PORT:-8000}" --proxy-headers
    ;;
  worker)
    echo "[entrypoint] starting Celery worker"
    # важливо: модуль:об'єкт -> celery_app:celery_app
    exec celery -A celery_app:celery_app worker -l info
    ;;
  beat)
    echo "[entrypoint] starting Celery beat"
    exec celery -A celery_app:celery_app beat -l info --schedule "$SCHEDULE_DIR/celerybeat-schedule.db"
    ;;
  *)
